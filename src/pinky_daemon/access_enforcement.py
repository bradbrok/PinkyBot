...
The owner's standing security rule (2026-09-12): staff must never gain tools they
should not have, or restricted information, simply by asking an agent. Today the
company access model (``permissions.json``) is enforced ONLY by the
external tools page; it has zero code consumers in the daemon, so a
staffer who free-texts the agent in Slack is bounded only by prose in his soul,
...
Model shape (``permissions.json``):
  * ``departments`` maps each group (e.g. Sales, Helpdesk, Support, Admin,
    SuperAdmin) to a list of ``{name, email}`` members.
  * ``systems`` are the web apps/pages; ``access`` is the agent's tool kit, every one a ``use``-gated item.
  ...
  * An item marked ``public: true`` (e.g. a public booking form) is open to anyone.

Enforcement policy (differs from the external tools-page resolver on purpose):
  * ADMINS are ALWAYS allowed, independent of the model, so a bad or empty model
    can never lock the owners (or the automation identity) out.
  * The AGENT path is FAIL-CLOSED, per the owner's rule "no group = no access except
...
def tool_keys(model: dict) -> list[str]:
    """Return the sorted keys of the agent's tool kit (the ``access`` section)."""
