...
Usage:
    ...
    python -m pinky_memory --transport stdio
"""
...
def main() -> None:
    ...
    parser.add_argument("--db", default=os.environ.get("PINKY_MEMORY_DB", "data/memory.db"),
                        help="Path to SQLite database (default: data/memory.db)")
    ...
    print(f"[pinky-memory] db={args.db} transport={args.transport}", file=sys.stderr, flush=True)
