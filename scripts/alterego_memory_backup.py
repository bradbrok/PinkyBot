#!/usr/bin/env python3
"""
alterego_memory_backup.py — Backup automatico messaggi alter-ego nel DB delle memorie.

Gira ogni 10 minuti via cron. Legge i messaggi di Mirko dalla chat-history API
del broker e li salva via HTTP API (interview-response endpoint) — INDIPENDENTEMENTE
da se l'agente ha chiamato ingest_memory o no.

Questo è il safety net: anche se alter-ego non salva, questo script salva.

Log: /home/pinky/.pinkybot/scripts/alterego_memory_backup.log
"""
...
TRACKER_FILE = "/home/pinky/.pinkybot/scripts/alterego_backup_state.json"
LOG_PATH = "/home/pinky/.pinkybot/scripts/alterego_memory_backup.log"
...
def extract_mirko_content(raw_content):
    """Estrae il testo pulito dal messaggio broker di Mirko."""
    # Formato: [telegram | dm | Ziomik... | 32405655 | timestamp | msg_id:NNN]\ntesto
    lines = raw_content.strip().split("\n")
    # Rimuovi la prima riga (header broker) e le righe di reply hint
    cleaned = []
    skip_next = False
    for line in lines:
        if line.startswith("[telegram |") and MIRKO_CHAT_ID in line:
            continue  # header
...
def detect_category(content):
    """Rileva categoria dal contenuto con keyword matching."""
    content_lower = content.lower()

    # Relationships
    if any(kw in content_lower for kw in ["mamma", "papà", "famiglia", "valentina", "amici"]):
...
def save_message_fallback(cur, msg_id, content, timestamp):
    ...
    """, (
        ...
        "broker_backup",
        f"broker_{msg_id}",
        "conversazione,backup_automatico"
    ))
...
def main():
    ...
    # Filtra: solo user messages di Mirko, più recenti dell'ultimo salvato
    new_msgs = [
        m for m in messages
        if m.get("role") == "user"
        and MIRKO_CHAT_ID in m.get("content", "")
        and "Heartbeat" not in m.get("content", "")
        and m["id"] > last_id
    ]

    if not new_msgs:
        log(f"Nessun nuovo messaggio da salvare (last_id={last_id}).")
        return
    ...
    for msg in sorted(new_msgs, key=lambda x: x["id"]):
        ...
        content = extract_mirko_content(raw_content)
    ...
    if saved > 0:
        log(f"BACKUP: salvati {saved} messaggi nuovi (saltati {skipped}).")
    else:
        log(f"BACKUP: tutto già salvato (saltati {skipped}).")
