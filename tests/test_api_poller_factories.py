...
def test_api_factories_wire_owner_notify(tmp_path, monkeypatch):
    ...
    agents.set_token("retry-test", "discord", "test-discord", settings={
        "poll_interval_sec": 2.5, "watched_channels": ["test-channel"],
    })
    try:
        with TestClient(app) as client:
            ...
            for platform in ("telegram", "discord"):
                response = client.put(f"/agents/retry-test/tokens/{platform}", json={
    ...
    for platform, constructor in (
        ("telegram", "BrokerTelegramPoller"), ("discord", "BrokerDiscordPoller"),
    ):
        factory = f"_new_{platform}_broker_poller"
        nodes = [n for n in ast.walk(tree) if isinstance(n, ast.Call)]
        constructions = [n for n in nodes if isinstance(n.func, ast.Name) and n.func.id == constructor]
        assert len(constructions) == 1
        definition = next(n for n in ast.walk(tree) if isinstance(n, ast.FunctionDef) and n.name == factory)
        assert constructions[0] in list(ast.walk(definition))
        for caller in ("set_agent_token", "on_startup"):
            body = next(n for n in ast.walk(tree) if isinstance(n, ast.AsyncFunctionDef) and n.name == caller)
            assert any(isinstance(n, ast.Call) and isinstance(n.func, ast.Name) and n.func.id == factory
