...
async def test_complete_next_task_appended_during_idle_does_not_accept(harness, capture_number):
    from tests.test_codex_scheduler_idle_receipt import IDLE, _ok
...
@pytest.mark.asyncio
@pytest.mark.parametrize("boundary", ["parent_callback", "confirmation_delay"])
async def test_real_repaste_of_same_turn_revokes_old_completion(harness, monkeypatch, boundary):
    turn, receipt = await _paste(harness)
    await _start(harness)
    ...
    if boundary == "parent_callback":
        ...
        async def sleep(delay):
            if harness.session._tailer.completion is not None:
                await repaste()
